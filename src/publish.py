...
def upload_artifact_file(artifact_file_path, portal_token):
    ...
    while poll_count < max_polls:
        deployment_state, detail = query_status(response.text, portal_token)
        if deployment_state == "PENDING":
            poll_count += 1
            time.sleep(poll_interval)
        elif deployment_state == "FAILED":
            raise Exception(f"Upload artifact file error! Detail: {detail}")
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        log_r(f"Error parsing artifact_path_list parameter: {e}")
        exit(1)
    query_status_dict = {}
    query_id_dict = {}
    log("Start uploading...")
